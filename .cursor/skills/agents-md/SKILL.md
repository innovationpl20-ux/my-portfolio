...
name: agents-md
description: >-
  AGENTS.md 거버넌스 시스템을 분석·생성하는 마스터 프롬프트. 현재 프로젝트를 분석하여 루트 AGENTS.md와 하위
  AGENTS.md를 즉시 생성하고, CLAUDE.md에 @AGENTS.md 링크를 추가한다. "AGENTS.md 만들어줘",
  "에이전트 규칙 만들어줘", "/agents-md" 호출 시 반드시 실행하라.
...
2. **No Fluff, No Emojis:** 컨텍스트 낭비를 막기 위해 **이모지와 불필요한 서술을 절대 사용하지 마십시오.** 오직 명확하고 간결한 텍스트로만 작성합니다.
...
5. **No Duplication:** README, docs/, 기존 문서에 이미 있는 내용을 반복하지 마십시오. AGENTS.md는 **기존 문서에 없는 에이전트 전용 지침만** 포함합니다.
...
- **Project Context (간결하게)**
  ...
  - **코드베이스 개요(Architecture Overview) 금지** — 디렉토리 구조 나열은 비용만 증가시킨다.
...
다음 신호가 감지될 때 별도의 `AGENTS.md`를 생성합니다:
...
- **Module Context:** 해당 모듈의 역할과 의존성 관계 정의 (1~2문장).
...
### Nested File Anti-Patterns

- 루트 `AGENTS.md`의 내용을 하위 파일에 **반복하지 마라**.
- 해당 디렉토리의 **파일 목록을 나열하지 마라**.
...
3. **Markdown Only:** 생성되는 파일 내용은 유효한 Markdown 문법이어야 하며, 불필요한 설명 없이 작성하십시오.

---

**Command:** Analyze the current project immediately and **EXECUTE the creation** of the optimized `./AGENTS.md` system. For every `AGENTS.md` file created, ensure the corresponding `CLAUDE.md` links it via `@AGENTS.md`. Ensure **NO EMOJIS** are used to maximize context efficiency.
